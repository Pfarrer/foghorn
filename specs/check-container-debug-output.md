...
## Usage Steps
1. Set `check_container_debug_output` globally or per check (`off`, `on_failure`, `always`).
2. Run Foghorn with debug logging.
3. Inspect redacted container stdout/stderr in daemon logs.

## Implementation Notes
- Extend config with `check_container_debug_output` mode (global default and per-check override).
...
## Acceptance Criteria
- [x] Config supports `check_container_debug_output: off|on_failure|always` globally and per check.
